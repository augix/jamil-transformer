...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=3):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            print_msg(f"{f'Initial: ':>12}{source_text}")
...
def get_ds(config):
    import pickle
    with open('../../AGRN/useGNW/analysis0829/res.s3/state_transition.pkl', 'rb') as f:
        ds_raw = pickle.load(f)
...
def get_model(config, vocab_src_len, vocab_tgt_len):
    model = build_transformer(vocab_src_len, vocab_tgt_len, config["seq_len"], config['seq_len'], N=config['num_layers'], h=config['num_heads'], d_model=config['d_model'], dropout=config['dropout'])
    return model
